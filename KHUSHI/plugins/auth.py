"""KHUSHI — Auth Users: /auth, /unauth, /authusers"""
...
from KHUSHI.utils.inline import close_markup
from config import BANNED_USERS, adminlist


@app.on_message(filters.command("auth") & filters.group & ~BANNED_USERS)
@AdminActual
async def auth_user(client, message: Message, _):
...
@AdminActual
async def unauth_user(client, message: Message, _):
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text(_["general_1"])
    user = await extract_user(message)
...
@language
async def auth_users_list(client, message: Message, _):
